...
// TestRenderJSONAllEightSliceFieldsAreArrays covers every slice in the schema,
...
// The count is in the name deliberately: a slice added to the schema without a
// row below is a field this gate silently stops covering, so the number has to
// be edited to add one.
func TestRenderJSONAllEightSliceFieldsAreArrays(t *testing.T) {
	o := &syncOutcome{
		Artifacts: &artifactsSection{Skipped: strs(nil), Warnings: strs(nil), Failures: strs(nil)},
		Seeds:     &blocksSection{Warnings: strs(nil), Failures: strs(nil)},
		Rules:     &blocksSection{Warnings: strs(nil), Failures: strs(nil)},
		Report:    &reportSection{Warnings: strs(nil)},
	}
	var buf bytes.Buffer
	if err := renderJSON(&buf, o); err != nil {
		t.Fatal(err)
...
// A report warning reaches the terminal, and it reaches it as a WARNING.
// The human renderer is the only surface most users ever read, so a failure
// that shows up in --json alone is a failure nobody sees.
func TestRenderHumanPrintsReportWarningsAsWarnings(t *testing.T) {
	o := &syncOutcome{
		Artifacts: &artifactsSection{Handled: 1, Added: 1, Skipped: strs(nil), Warnings: strs(nil), Failures: strs(nil)},
		Report:    &reportSection{Warnings: []string{"the deployment report did not reach http://api: status 500"}},
	}
	var buf bytes.Buffer
	renderHuman(&buf, o)
	got := buf.String()
	if !strings.Contains(got, "  warning: the deployment report did not reach http://api: status 500\n") {
		t.Fatalf("the report warning is not in the human output:\n%s", got)
	}
	if strings.Contains(got, "failed:") {
		t.Fatalf("a failed report must not render as a failure:\n%s", got)
	}
}

// A clean report prints nothing at all: the run did what was asked, and a
// per-run line about bookkeeping is noise on the surface people read daily.
func TestRenderHumanIsSilentOnACleanReport(t *testing.T) {
	o := &syncOutcome{
		Artifacts: &artifactsSection{Handled: 1, Unchanged: 1, Skipped: strs(nil), Warnings: strs(nil), Failures: strs(nil)},
		Report:    &reportSection{Recorded: 1, Dropped: 0, Warnings: strs(nil)},
	}
	var buf bytes.Buffer
	renderHuman(&buf, o)
	want := "Synced 1 artifact(s): 0 added, 0 updated, 1 unchanged, 0 removed.\n"
	if got := buf.String(); got != want {
		t.Fatalf("renderHuman on a clean report:\n--- got ---\n%s\n--- want ---\n%s", got, want)
	}
}
